...
export function useBookings(limit = 100, initialOffset = 0) {
  ...
  const createBooking = async (booking: Omit<Booking, 'id'>) => {
    if (!isAuthenticated) throw new Error('You must be logged in to create a booking');
    try {
      setLoading(true);
  ...
  const updateBooking = async (id: string, booking: Partial<Booking>) => {
    if (!isAuthenticated) throw new Error('You must be logged in to update a booking');
    try {
      setLoading(true);
  ...
  const deleteBooking = async (id: string) => {
    if (!isAuthenticated) throw new Error('You must be logged in to delete a booking');
    try {
      setLoading(true);
  ...
  const refundBooking = async (bookingId: string) => {
    if (!isAuthenticated) throw new Error('You must be logged in to request a refund');
    try {
      setLoading(true);
      await paymentApi.refund(bookingId);
      setBookings((prev) =>
        prev.map((b) =>
          b.id === bookingId ? { ...b, status: 'cancelled', payment_status: 'refunded' } : b
        )
      );
    } catch (err) {
      setError(err instanceof Error ? err : new Error('Failed to process refund'));
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const nextPage = () => {
    if (response && response.data.length === limit) setOffset((prev) => prev + limit);
  };
